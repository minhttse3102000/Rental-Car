 ...
 * @author minhv
 */
public class SendRatingController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            orderDetailDAO.sendRating(orderDetailID,Integer.parseInt(rating));
            for (CarDTO carDTO : listDetail) {
                if (carDTO.getCarID().equals(orderDetailID)) {
                    carDTO.setRating(rating);
                }
            }

            url = SUCCESS;
            request.setAttribute("MESSAGE", "Send rating successful");
            session.setAttribute("LIST_DETAIL", listDetail);


        } catch (Exception e) {
            log("Error at SendRatingServlet " + e.toString());
        } finally {
